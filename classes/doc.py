...
class Doc:
    ...
    def get_straight_table_box_handles(self) -> tuple[StraightTableBox, ListBox, ListBox]:
        """
        ...
        if 'Запасы' in self.path:
            ...
            turnover_straight_table_box_index = 0 if ('ГИППО' in self.path) and ('Archive' not in self.path) else 1 
            logger.info(f'Getting straight table box at index {turnover_straight_table_box_index}.')
            stbox = stboxes[turnover_straight_table_box_index]
            stbox = StraightTableBox(stbox)
        else:
            stbox = (
                sh.get_straight_table_box('Универсальная таблица')
