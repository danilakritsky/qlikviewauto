...
class Field:
    ...
    def __init__(self, com: object) -> None:
        ...
        self.max_value_count = (
            com.GetValueCount(1)
            + com.GetValueCount(2)  # 1 - selected - fields selected directly
            + com.GetValueCount(  # 2 - optional - all avilable values - all values when no values are selected, 0 when a value is selected, possible values when other fields are cross filtering the field
                3
            )
            + com.GetValueCount(4)  # 3 - deselected
            + com.GetValueCount(  # 4 - alternative - (=other than selected) 0 - when none are selected, count of possible values when a value is selected)
                5
            )  # 5 - excluded -  all fields that are removed by other fields' cross-filtering
        )
    ...
    def deselect_values(self, *args) -> None:
        ...
        selected_values = {
            selected_values[i].Number if is_numeric else selected_values[i].Text
            for i in range(selected_values.Count)
        }
